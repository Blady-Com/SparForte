...
Graph : Gnoga.Gui.Plugin.Pixi.Graphics.Graphics_Access;

function ARGB_To_RGBA (r, g, b : ARGBComponent) return Gnoga.Types.RGBA_Type is
     ((Gnoga.Types.Color_Type (r * 255.0 / 100.0),
       Gnoga.Types.Color_Type (g * 255.0 / 100.0),
       Gnoga.Types.Color_Type (b * 255.0 / 100.0),
                               1.0));
...
begin
  Graph.Line_Color (ARGB_To_RGBA (ColourNames (c).red, ColourNames (c).green, ColourNames (c).blue));
end setPenInk;
...
begin
  ...
  if canvasIndex = 0 then
     put_line( Current_Error, "no such canvas id -" & canvas_id'img );
  else
...
--put_line( "Calling SDL_MapRGB" ); -- DEBUG
  ...
  Graph.Line_Color (ARGB_To_RGBA (r, g, b));
end setPenInk;
...
begin
  ...
  if brushIndex = 0 then
     put_line( Current_Error, "no such canvas id -" & pattern_id'img );
  else
...
--    res_int := SDL_Init( SDL_INIT_VIDEO );
  if res_int < 0 then
     put_line( Current_Error, "startupPen: sdl_init failed, SDL error = " & to_string( get_sdl_error ) );
  else
    --res := TTF_Init;
    --if res = SDL_Failed then
    --   put_line( Current_Error, "startupPen: TTF_init failed, SDL error = " & to_string( get_sdl_error ) );
    --   SDL_Quit;
    --else
...
--          put_line( Current_Error, "SDL_LockSurface failed" );
--       end if;
--    end if;
...
begin
  Graph.Draw_Rect (Integer (r.left), Integer (r.top), Integer (r.right - r.left),
                   Integer (r.bottom - r.top));
end frameRect;
...
begin
Graph.Begin_Fill(ARGB_To_RGBA(r, g, b));
Graph.Draw_Rect(Integer (theRect.left), Integer (theRect.top), Integer (theRect.right - theRect.left),
                Integer (theRect.bottom - theRect.top));
Graph.End_Fill;
end fillRect;
...
procedure paintRect( theCanvas : aCanvas; r : ARect ) is
  RC, GC, BC : aRGBComponent;
begin
getPenInk (theCanvas.id, RC, GC, BC);
Graph.Begin_Fill (ARGB_To_RGBA(RC, GC, BC));
Graph.Draw_Rect (Integer (r.left), Integer (r.top), Integer (r.right - r.left),
                Integer (r.bottom - r.top));
Graph.End_Fill;
end paintRect;
...
begin
Graph.Draw_Ellipse (Integer (r.left), Integer (r.top),
                       Integer (r.right - r.left),
                       Integer (r.bottom - r.top));
end frameEllipse;
...
getPenInk (theCanvas.id, RC, GC, BC);
Graph.Begin_Fill (ARGB_To_RGBA(RC, GC, BC));
Graph.Draw_Ellipse (Integer (r.left), Integer (r.top), Integer (r.right - r.left),
                   Integer (r.bottom - r.top));
Graph.End_Fill;
end paintEllipse;
...
begin
Graph.Begin_Fill(ARGB_To_RGBA(r, g, b));
Graph.Draw_Ellipse(Integer (theRect.left), Integer (theRect.top), Integer (theRect.right - theRect.left),
                Integer (theRect.bottom - theRect.top));
Graph.End_Fill;
end fillEllipse;
...
--    newcanvas.surface_ptr := SDL_Surface_Conv.To_Pointer( newcanvas.surface );
  if newcanvas.surface_ptr = null then
     put_line( Current_Error, "newScreenCanvas: failed for " & H_Res'img & V_Res'img & C_Res'img & "SDL error = " & to_string( get_sdl_error ) );
  end if;
...
--    res := SDL_GL_SetAttribute( SDL_GL_DOUBLEBUFFER, 1 );
  if res /= SDL_OK then
     put_line( Current_Error, "newGLWindowCanvas: double buffer failed" );
  end if;
  if newcanvas.surface_ptr = null then
     put_line( Current_Error, "newScreenCanvas: failed for " & H_Res'img & V_Res'img & C_Res'img & "SDL error = " & to_string( get_sdl_error ) );
  end if;
...
begin
  ...
  -- Workaround as no primitive exists to set the width of the pen
  Graph.Line_Width (C_Res);
...
--    newCanvas.surface_ptr := SDL_Surface_Conv.To_Pointer( newcanvas.surface );
--    if newCanvas.surface_ptr = null then
--       put_line( Current_Error, "newWindowCanvas: failed for " & H_Res'img & V_Res'img & C_Res'img & "SDL error = " & to_string( get_sdl_error ) );
--    end if;
...
--    newCanvas.surface_ptr := SDL_Surface_Conv.To_Pointer( newcanvas.surface );
  if newCanvas.surface_ptr = null then
     put_line( Current_Error, "newCanvas: failed for " & H_Res'img & V_Res'img & "SDL error = " & to_string( get_sdl_error ) );
  end if;
...
--    newcanvas.surface_ptr := SDL_Surface_Conv.To_Pointer( newcanvas.surface );
  if newcanvas.surface_ptr /= null then
    ...
    put_line( Current_Error, "Load failed, SDL error = " & to_string( get_sdl_error ) );
  end if;
  newcanvas.kind := offscreen;
  newcanvas.drawingType := raster;
...
--          res := SDL_EXT_Save_BMP( theCanvas.surface, path & ASCII.NUL );
        if res /= 0 then
           put_line( Current_Error, "unable to save canvas id -" & canvas_id'img );
        end if;
     end if;
  end if;
end saveCanvas;
...
begin
  Graph.Move_To (Integer (x), Integer (y));
  theCanvas.pen.x := x;
  theCanvas.pen.y := y;
end moveTo;
...
begin
  Graph.Move_To (Integer (theCanvas.pen.x + dx), Integer (theCanvas.pen.y + dy));
...
begin
  Graph.Line_To (Integer (x), Integer (y));
  theCanvas.pen.x := x;
  theCanvas.pen.y := y;
end lineTo;
...
begin
  Graph.Line_To (Integer (theCanvas.pen.x + dx), Integer (theCanvas.pen.y + dy));
  theCanvas.pen.x := theCanvas.pen.x + dx;
  theCanvas.pen.y := theCanvas.pen.y + dy;
end line;
...
begin
  Graph.Move_To (Integer (x1), Integer (y));
  Graph.Line_To (Integer (x2), Integer (y));
end hline;
...
begin
  Graph.Move_To (Integer (x), Integer (y1));
  Graph.Line_To (Integer (x), Integer (y2));
end vline;
...
--    surface_ptr := SDL_Surface_Conv.To_Pointer( scaledSurface );
  if surface_ptr = null then
     put_line( Current_Error, to_string( get_sdl_error ) );
     return;
  end if;
...
--    res := SDL_UpperBlit( scaledSurface, source_range, targetCanvas.surface, target_range );
--    if res < 0 then
--       put( Current_Error, "SDL_UpperBlit blit failed: " & to_string( get_sdl_error ) );
...
put_line( "lock" );
--            if SDL_LockSurface( theCanvas.surface  ) /= SDL_OK then
--               put_line( Current_Error, "unable to lock SDL surface" );
--            end if;
       end if;
put_line( "rect" );
...
begin
  ...
  if canvasIndex = 0 then
   ...
   procedure Set_Graphic (Graphic : Gnoga.Gui.Plugin.Pixi.Graphics.Graphics_Access) is
   begin
      Graph := Graphic;
   end Set_Graphic;
